...
<div class="content-wrapper flex justify-center items-center min-h-screen bg-gray-900">
    <div class="bg-gray-800 rounded-lg p-8 w-full md:w-2/3 lg:w-1/2 shadow-lg">
        ...
        <!-- Form -->
        <form method="post" action="{{ route('apparels.update', $apparel->id) }}" id="insertFormApparel" enctype="multipart/form-data">
            @method('PUT')
            @csrf
        ...
        <div class="mb-6 text-center">
            <div class="flex justify-center space-x-4">
                @foreach ($apparelCategories as $category)
                    <input type="radio" id="category_{{ $category->id }}" name="apparel_category_id" value="{{ $category->id }}" {{ $category->id == $apparel->apparel_category_id ? 'checked' : ''}} class="hidden peer/category_{{ $category->id }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="form-group">
                    ...
                    <select class="form-select bg-gray-700 text-white w-full" name="apparel_type_id" id="apparel_type_id" required>
                        ...
                        @foreach ($apparelTypes as $type)
                            <option value="{{ $type->id }}" {{$type->id == $apparel->apparel_type_id ? 'selected' : ''}}>{{ $type->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-select bg-gray-700 text-white w-full" name="apparel_style_id" id="apparel_style_id" required>
                        ...
                        @foreach ($apparelStyles as $style)
                            <option value="{{ $style->id }}"{{$style->id == $apparel->apparel_style_id ? 'selected' : ''}}>{{ $style->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                name="date" 
                id="date" 
                value="{{ $apparel->getDate($apparel->date) }}"
                required
                />
