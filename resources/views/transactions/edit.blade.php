...
<div class="content-wrapper flex justify-center items-center min-h-screen bg-gray-900">
    <div class="bg-gray-800 rounded-lg p-8 w-full md:w-2/3 lg:w-1/2 shadow-lg">
        <h2 class="text-2xl font-bold mb-6 text-white text-center">Edit Transaction</h2>
        ...
        <!-- Form -->
        <form method="post" action="{{ route('transactions.update', $transaction->id) }}" id="insertFormApparel" enctype="multipart/form-data">
            @method('PUT')
            @csrf
        ...
        <div class="mb-6 text-center">
            <div class="flex justify-center space-x-4">
                    <input type="radio" id="Expense" name="type" value="Expense" class="appearance-none" {{ $value == $transaction->type ? 'checked' : ''}}>
                    <label for="Expense" class="px-4 py-2 bg-gray-700 text-white border-3 rounded-full cursor-pointer border-indigo-900">Expense</label>
                    <input type="radio" id="Income" name="type" value="Income" class="appearance-none" {{ $value !== $transaction->type ? 'checked' : ''}}>
                    <label for="Income" class="px-4 py-2 bg-gray-700 text-white border-3 rounded-full cursor-pointer border-indigo-900">Income</label>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="form-group">
                    <label for="apparel_category_id" class="form-label text-white">Category</label>
                    <select class="form-select bg-gray-700 text-white w-full" name="transaction_category_id" id="transaction_category_id" required>
                        ...
                        @foreach ($transactionCategories as $category)
                            <option value="{{ $category->id }}" {{$category->id == $transaction->transaction_category_id ? 'selected' : ''}}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                name="date" 
                id="date" 
                value="{{ $transaction->getDate($transaction->date) }}"
                required
                />
